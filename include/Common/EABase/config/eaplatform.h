...
//
// Other definitions emanating from this file include:
...
#if defined(EA_PLATFORM_EXTERNAL)
	// EA_PLATFORM_EXTERNAL is a pseudo-platform which is defined by external build systems. It allows the build system to define a new platform rather than relying on this file.
...
// WASM (WebAssembly)
// EMSCRIPTEN is defined by the emscripten compiler.
#elif defined(EA_PLATFORM_WASM) || defined(EMSCRIPTEN)
	#define EA_PLATFORM_UNIX 1
	#define EA_PLATFORM_POSIX 1
	#define EA_PLATFORM_NAME "Wasm"
	#define EA_PROCESSOR_WASM 1
	#define EA_SYSTEM_LITTLE_ENDIAN 1
	#define EA_PLATFORM_DESCRIPTION "Web Assembly"
	#define EA_PLATFORM_DESKTOP 1
